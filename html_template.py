...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        body { background: #080b12; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif; color: #fff; overflow-x: hidden; width: 100%; -webkit-overflow-scrolling: touch; padding-bottom: 80px; } 
        
        header { display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 12px 10px; border-bottom: 1px solid #1e293b; position: sticky; top: 0; background: rgba(8, 11, 18, 0.95); backdrop-filter: blur(10px); z-index: 1000; width: 100%; gap: 8px; }
        ...
        .bottom-nav { position: fixed; bottom: 0; left: 0; width: 100%; background: rgba(8, 11, 18, 0.98); backdrop-filter: blur(15px); border-top: 1px solid #1e293b; display: flex; justify-content: space-around; align-items: center; padding: 10px 0; z-index: 2000; padding-bottom: calc(10px + env(safe-area-inset-bottom)); }
        ...
        .search-input { width: 100%; padding: 16px; border-radius: 25px; border: none; outline: none; text-align: center; background: #161f30; color: #fff; font-size: 17px; font-weight: bold; }
        ...
        .sparkle-icon { display: inline-block; font-size: 18px; font-family: serif; color: #7c3aed; line-height: 1; }
        ...
        .ad-carousel-dots { display: flex; gap: 5px; margin-top: 5px; justify-content: center; }
        .ad-carousel-dot { width: 6px; height: 6px; border-radius: 50%; background: #475569; }
        .ad-carousel-dot.active { background: #ff4e2a; transform: scale(1.25); }
        ...
        .post-content { position: relative; width: 100%; border-radius: 11px; overflow: hidden; }
        .post-content img { width: 100%; aspect-ratio: 16/9; height: auto; object-fit: cover; display: block; }
        ...
        .page-btn { background: #161f30; color: #fff; border: 1px solid #334155; padding: 8px 14px; border-radius: 6px; cursor: pointer; font-weight: bold; }
        ...
        .modal { position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.85); display: none; align-items: center; justify-content: center; z-index: 7000; backdrop-filter: blur(5px); }
        .modal-content { background: #161f30; width: 92%; max-width: 400px; padding: 25px; border-radius: 20px; text-align: center; border: 1px solid #334155; max-height: 85vh; overflow-y: auto; position: relative; }
        ...
        .ep-badge { bottom: 10px; right: 10px; background: #10b981; }
        .lang-card-badge { top: 10px; right: 10px; background: linear-gradient(135deg, #ef4444, #dc2626); color: #ffffff !important; font-size: 10px; font-weight: 900; padding: 4px 10px; text-transform: uppercase; }
        ...
        .details-nav-title { font-size: 18px; font-weight: 800; color: #f1f5f9; }
        ...
        .details-content-body { padding: 18px 16px; }
        .details-movie-title { font-size: 19px; font-weight: 800; color: #ffffff; line-height: 1.4; margin-bottom: 20px; }
        ...
        .stat-item .stat-num { font-size: 16px; font-weight: 800; color: #f8fafc; }
        .stat-item .stat-lbl { font-size: 11px; color: #94a3b8; font-weight: 600; }
        ...
        .action-btn-pill.liked { color: #ef4444; border-color: rgba(239, 68, 68, 0.4); background: rgba(239, 68, 68, 0.1); }
        .action-btn-pill.saved { color: #fbbf24; border-color: rgba(251, 191, 36, 0.4); background: rgba(251, 191, 36, 0.1); }
        ...
        .rating-stars-interactive { display: flex; gap: 6px; color: #475569; font-size: 18px; cursor: pointer; }
        .rating-stars-interactive i.active-star { color: #fbbf24; }
        .rating-score-display { color: #fbbf24; font-weight: 900; font-size: 14px; display: flex; align-items: center; gap: 4px; }

        /* কমেন্টস সেকশন */
        .comments-section-wrap { margin-top: 10px; }
        .comments-header-title { font-size: 17px; font-weight: 800; color: #ffffff; margin-bottom: 15px; }
        ...
        .comment-body-right { flex-grow: 1; }
        .comment-author-name { font-size: 14px; font-weight: 800; color: #ffffff; display: flex; align-items: center; gap: 6px; }
        .comment-admin-badge { background: #f59e0b; color: #000; font-size: 10px; font-weight: 900; padding: 2px 6px; border-radius: 4px; }
        .comment-time-ago { font-size: 11px; color: #94a3b8; font-weight: normal; margin-left: auto; }
        
        .comment-reply-to-text { font-size: 11.5px; color: #38bdf8; font-weight: 700; margin-top: 2px; }
        .comment-text-msg { font-size: 13.5px; color: #cbd5e1; line-height: 1.5; margin-top: 4px; margin-bottom: 8px; }

        .comment-reply-bar { display: flex; align-items: center; gap: 16px; font-size: 12px; color: #94a3b8; font-weight: 700; }
        .comment-reply-bar span { cursor: pointer; display: flex; align-items: center; gap: 4px; }
        ...
        /* ফিক্সড কমেন্ট ইনপুট বক্স */
        .comment-dock-container {
            position: fixed;
            bottom: 0;
            left: 0;
            width: 100%;
    ...
    <div id="movieDetailScreen">
        ...
        <div class="details-content-body">
            ...
            <div class="stats-counter-bar">
                ...
                <div class="stat-item" onclick="focusCommentBox()">
        ...
        <div class="comment-dock-container">
            <div id="replyTargetBanner">
                <span id="replyTargetText">Replying to User</span>
        ...
        async function likeComment(commentId, btnElement) {
            try {
                const res = await fetch('/api/comments/like', {
                    method: 'POST',
                    headers: {'Content-Type': 'application/json'},
                    body: JSON.stringify({ uid: uid, comment_id: commentId, initData: INIT_DATA })
                });
                const d = await res.json();
                if (d.ok) {
                    btnElement.querySelector('.c-likes').innerText = d.likes;
                }
            } catch(e) {}
        }
        ...
        async function sendFileAndClose(id) {
            ...
            try {
                ...
                if(data.ok) { 
                    ...
                    hideProcessingUI();
                    tg.showAlert("⚠️ Session expired! Please reopen the mini app.");
                }
            } catch (e) {
                hideProcessingUI();
